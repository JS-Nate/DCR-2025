...
def preprocess_to_5s_groupaware_v3(input_csv: str, output_csv: str, smooth_eye=True) -> pd.DataFrame:
    ...
    col_gaze = pick(["Eye Detection","eye_detection","eye_tracking","eye tracking","eye_gaze","gaze","eye direction","eye movement","gaze token"])
    if col_gaze:
    ...
    col_emotion = pick(["Emotion Detection","emotion_detection","emotion","Face Emotion"])
    if col_emotion:
    ...
    order_front = [c for c in desired if c in grouped.columns]
